...
func TestSupervisor_Supervise(t *testing.T) {
	t.Run("Process exits successfully", func(t *testing.T) {
		...
		// Create supervisor
		supervisor := NewSupervisor(spawner, SupervisorConfig{}, logger)
	...
	t.Run("Process fails and gets restarted", func(t *testing.T) {
		...
		supervisor := NewSupervisor(spawner, SupervisorConfig{
			AutoRestart:       true,
...
func TestCalculateBackoffDelay(t *testing.T) {
	t.Run("Initial backoff", func(t *testing.T) {
		delay := CalculateBackoffDelay(0, 1000, 30000)
	...
	t.Run("Exponential backoff", func(t *testing.T) {
		...
		delay := CalculateBackoffDelay(3, 1000, 30000)
	...
	t.Run("Maximum backoff cap", func(t *testing.T) {
		...
		delay := CalculateBackoffDelay(10, 1000, 30000)
	...
	t.Run("Different initial values", func(t *testing.T) {
		delay := CalculateBackoffDelay(2, 500, 10000)
...
func TestNewSupervisor(t *testing.T) {
	t.Run("Default configuration", func(t *testing.T) {
		...
		assert.Equal(t, spawner, supervisor.spawner)
		assert.Equal(t, logger, supervisor.logger)
		assert.Equal(t, 10, supervisor.config.MaxRestartAttempts) // Default value
		assert.Equal(t, 1000, supervisor.config.InitialBackoffMs) // Default value
		assert.Equal(t, 30000, supervisor.config.MaxBackoffMs)    // Default value
	})
	...
	t.Run("Custom configuration", func(t *testing.T) {
		...
		supervisor := NewSupervisor(spawner, config, logger)
		require.NotNil(t, supervisor)
		
		// Verify custom values
		assert.Equal(t, true, supervisor.config.AutoRestart)
		assert.Equal(t, 5, supervisor.config.MaxRestartAttempts)
		assert.Equal(t, 2000, supervisor.config.InitialBackoffMs)
		assert.Equal(t, 20000, supervisor.config.MaxBackoffMs)
	})
}